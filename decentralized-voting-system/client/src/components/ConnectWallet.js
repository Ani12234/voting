...
import { VOTER_REGISTRY_CONTRACT_ADDRESS } from '../config';
      ...
      if (accounts.length > 0) {
        ...
        try {
          ...
          if (!isRegistered) {
            ...
            try {
              const response = await fetch(`${process.env.REACT_APP_API_URL || 'http://localhost:5000'}/api/voters/status/${accounts[0]}`);
              if (response.ok) {
                const data = await response.json();
                if (data.pending) {
